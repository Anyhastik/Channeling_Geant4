...
class G4ChanRad : public G4VDiscreteProcess
{
 public:
  explicit G4ChanRad(G4LogicalVolume* anEnvelope, G4Material*,
                            G4Material*, G4double, G4double, G4int, G4double,
  ...
  virtual G4double SpectralXTRdEdx(G4int i, G4double energy, vector<ld1> emission, ld1 min_TT);
  ...
  static constexpr d num = n_steps * nbeam;
  static constexpr G4int poi = G4int (num); 
  
void GetTrajectories(vector<d> &x_path, vector<d> &time_moving, 
								vector<d> &x_velocity,
...
ld1 integrator_trap(ld1 y2, ld1 y1, ld1 w2, ld1 w1);
...
void GetPeriods(G4int sigen, G4int k0, G4int l0, G4int n0, G4String crystaltype);
 ...
 protected:
  ...
  //array<ld1, nbeam> TT;
    vector<ld1> TT;
	vector<d> x_path;
	vector<d>  time_moving;
	vector<d>  x_velocity;
	vector<d>  z_path;
	vector<d>  z_velocity;
	//copies of arrays
	vector<d> x_path_copy;
	vector<d> time_moving_copy;
	vector<d> x_velocity_copy;
	vector<d> z_path_copy;
	vector<d> z_velocity_copy;
	int tables_count;
///////////////////////////////////////////////
  G4bool fExitFlux;
